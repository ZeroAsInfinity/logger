...
const ENVIRONMENT_AI_POLICY_PRESETS = {
    dev: {
...
class Samiam {
    ...
    isAiCircuitOpen(now = Date.now()) {
        return now < this.aiCircuitOpenUntil;
    }
    markAiAttemptSuccess() {
        this.aiConsecutiveFailures = 0;
    }
    markAiAttemptFailure() {
    ...
    applyOperatingProfile(profile, preserveExplicit) {
        if (!['conservative', 'balanced', 'aggressive'].includes(profile)) {
            profile = 'balanced';
        }
        const shouldSet = (key) => !preserveExplicit || preserveExplicit[key] === undefined;
        if (profile === 'conservative') {
            if (shouldSet('minLevel'))
                this.config.minLevel = 'info';
            if (shouldSet('rateLimitMs'))
                this.config.rateLimitMs = 2500;
            if (shouldSet('minRateLimitMs'))
                this.config.minRateLimitMs = 250;
            if (shouldSet('maxRateLimitMs'))
                this.config.maxRateLimitMs = 90_000;
            if (shouldSet('aiMaxRetries'))
                this.config.aiMaxRetries = 1;
            if (shouldSet('adaptiveReviewLogThreshold'))
                this.config.adaptiveReviewLogThreshold = 400;
            if (shouldSet('aiCircuitBreakerFailures'))
                this.config.aiCircuitBreakerFailures = 2;
            if (shouldSet('aiCircuitBreakerCooldownMs'))
                this.config.aiCircuitBreakerCooldownMs = 45_000;
        }
        if (profile === 'balanced') {
            if (shouldSet('minLevel'))
                this.config.minLevel = 'debug';
            if (shouldSet('rateLimitMs'))
                this.config.rateLimitMs = 1500;
            if (shouldSet('minRateLimitMs'))
                this.config.minRateLimitMs = 100;
            if (shouldSet('maxRateLimitMs'))
                this.config.maxRateLimitMs = 60_000;
            if (shouldSet('aiMaxRetries'))
                this.config.aiMaxRetries = 2;
            if (shouldSet('adaptiveReviewLogThreshold'))
                this.config.adaptiveReviewLogThreshold = 200;
            if (shouldSet('aiCircuitBreakerFailures'))
                this.config.aiCircuitBreakerFailures = 3;
            if (shouldSet('aiCircuitBreakerCooldownMs'))
                this.config.aiCircuitBreakerCooldownMs = 30_000;
        }
        if (profile === 'aggressive') {
            if (shouldSet('minLevel'))
                this.config.minLevel = 'debug';
            if (shouldSet('rateLimitMs'))
                this.config.rateLimitMs = 700;
            if (shouldSet('minRateLimitMs'))
                this.config.minRateLimitMs = 75;
            if (shouldSet('maxRateLimitMs'))
                this.config.maxRateLimitMs = 25_000;
            if (shouldSet('aiMaxRetries'))
                this.config.aiMaxRetries = 3;
            if (shouldSet('adaptiveReviewLogThreshold'))
                this.config.adaptiveReviewLogThreshold = 100;
            if (shouldSet('aiCircuitBreakerFailures'))
                this.config.aiCircuitBreakerFailures = 4;
            if (shouldSet('aiCircuitBreakerCooldownMs'))
                this.config.aiCircuitBreakerCooldownMs = 20_000;
        }
    ...
    applyEnvironmentAiPolicy(environment, preserveExplicit) {
        const preset = ENVIRONMENT_AI_POLICY_PRESETS[environment];
        if (!preset)
            throw new TypeError('Invalid runtime environment');
        const shouldSet = (key) => !preserveExplicit || preserveExplicit[key] === undefined;
        this.config.runtimeEnvironment = environment;
        if (shouldSet('aiEngagementMode'))
            this.config.aiEngagementMode = preset.aiEngagementMode;
    ...
    shouldRunAiForStage(stage, details) {
    ...
    evaluateAndEmitAiEngagement(stage, details) {
        ...
        if (!decision.engage) {
            if (decision.reason === 'AI circuit is open')
    ...
    async requestAiWithRetry(prompt) {
        if (this.isAiCircuitOpen()) {
            this.statsData.aiCircuitOpenSkips++;
    ...
    defaultAiProvider = async (prompt, config, options) => {
    ...
    mergeUniqueLevels(current, incoming) {
    ...
    mergeUniqueSanitizeKeys(current, incoming) {
    ...
    normalizeAdaptation(value) {
        ...
        const raw = value;
        const out = {};
        if (this.isValidLevel(raw.minLevel))
            out.minLevel = raw.minLevel;
        if (Array.isArray(raw.notifyOnLevels)) {
            out.notifyOnLevels = raw.notifyOnLevels.filter((level) => this.isValidLevel(level));
        }
    ...
    normalizeSyncActions(value) {
        ...
        for (const item of value) {
            if (!item || typeof item !== 'object' || Array.isArray(item))
                continue;
            const raw = item;
            if (!this.syncActionTypes.has(raw.type))
                continue;
            if (this.config.strictSyncMode && raw.type === 'custom' && !this.config.allowCustomSyncActions)
                continue;
            const action = {
                type: raw.type,
                payload: raw.payload,
    ...
    setAiProvider(provider) {
        if (typeof provider !== 'function')
            throw new TypeError('AI provider must be a function');
        this.aiProvider = provider;
    }
    registerSyncHandler(handler) {
        if (typeof handler !== 'function')
            throw new TypeError('Sync handler must be a function');
    ...
    setOperatingProfile(profile) {
        ...
        if (previous !== profile)
            this.emitEvent('profileChanged', { previous, next: profile });
    }
    getOperatingProfile() { return this.config.operatingProfile; }
    static getLoggingPolicy(workload) {
        const preset = WORKLOAD_POLICY_PRESETS[workload];
        if (!preset)
            throw new TypeError('Invalid workload type');
        return {
            workload,
    ...
    static getQualityGuardPolicy(workload) {
    ...
    static getEnvironmentAiPolicy(environment) {
        const preset = ENVIRONMENT_AI_POLICY_PRESETS[environment];
        if (!preset)
            throw new TypeError('Invalid runtime environment');
        return {
    ...
    setRuntimeEnvironment(environment, enforcePolicy) {
        this.config.runtimeEnvironment = environment;
        if (typeof enforcePolicy === 'boolean')
            this.config.enforceEnvironmentAiPolicy = enforcePolicy;
        if (this.config.enforceEnvironmentAiPolicy)
            this.applyEnvironmentAiPolicy(environment);
    }
    getRuntimeEnvironment() {
        return this.config.runtimeEnvironment;
    }
    setEnvironmentAiPolicyEnforcement(enabled) {
        this.config.enforceEnvironmentAiPolicy = enabled;
        if (enabled)
            this.applyEnvironmentAiPolicy(this.config.runtimeEnvironment);
    }
    getEnvironmentAiPolicyEnforcement() {
        return this.config.enforceEnvironmentAiPolicy;
    }
    applyLoggingPolicy(policy) {
    ...
    getQualityMetrics(options = {}) {
        const recentLogCount = Number.isFinite(options.recentLogCount)
            ? Math.max(1, Math.floor(options.recentLogCount))
            : 200;
        const actionableLevels = (options.actionableLevels?.length
            ? options.actionableLevels
            : this.config.notifyOnLevels).filter((level) => this.isValidLevel(level));
        const noiseLevels = (options.noiseLevels?.length
            ? options.noiseLevels
            : ['debug', 'info']).filter((level) => this.isValidLevel(level));
        const recentLogs = this.getRecentLogs(recentLogCount);
        const actionableSet = new Set(actionableLevels);
        const noiseSet = new Set(noiseLevels);
        let actionableCount = 0;
        let noiseCount = 0;
        let errorCount = 0;
        let warnCount = 0;
        for (const entry of recentLogs) {
            if (actionableSet.has(entry.level))
                actionableCount++;
            if (noiseSet.has(entry.level))
                noiseCount++;
    ...
    recordQualitySnapshot(label = 'qol-snapshot', options = {}) {
    ...
    evaluateQualityGuard(options = {}) {
        ...
        if (current) {
            if (current.metrics.noiseRatio > resolvedPolicy.maxNoiseRatio) {
                violations.push(`Noise ratio ${current.metrics.noiseRatio.toFixed(3)} exceeds max ${resolvedPolicy.maxNoiseRatio.toFixed(3)}.`);
            }
            if (current.metrics.actionableAlertPrecision < resolvedPolicy.minActionableAlertPrecision) {
                violations.push(`Actionable precision ${current.metrics.actionableAlertPrecision.toFixed(3)} is below min ${resolvedPolicy.minActionableAlertPrecision.toFixed(3)}.`);
            }
        }
        if (current && baseline) {
            ...
            if (noiseIncrease > resolvedPolicy.maxNoiseRatioIncrease) {
                violations.push(`Noise ratio increased by ${noiseIncrease.toFixed(3)} (limit ${resolvedPolicy.maxNoiseRatioIncrease.toFixed(3)}).`);
            }
            if (precisionDrop > resolvedPolicy.maxActionablePrecisionDrop) {
                violations.push(`Actionable precision dropped by ${precisionDrop.toFixed(3)} (limit ${resolvedPolicy.maxActionablePrecisionDrop.toFixed(3)}).`);
            }
        }
    ...
    getAutonomousRecommendations(options = {}) {
    ...
    getTroubleshootingChecklist(topic = 'general') {
        switch (topic) {
            case 'ai-failures':
                return [
    ...
    namespace(prefix) {
        ...
        return {
            ...
            child: (nestedPrefix) => base.namespace(`${prefix}.${nestedPrefix}`),
    ...
    getRecentLogs(count = 50, level) {
        ...
        if (!level)
            return normalizedCount > 0 ? this.logs.slice(-normalizedCount) : [];
...
State: ${JSON.stringify(state)}`;
    ...
    async triggerAdaptiveReview(reason, force = false) {
            ...
            try {
                ...
                switch (action.type) {
                    ...
                    case 'logPattern':
                        ...
                        break;
                    case 'custom':
                        if (this.config.strictSyncMode && !this.config.allowCustomSyncActions)
                            break;
...
Context: ${JSON.stringify(context)}`
        ...
        try {
            ...
            if (parsed) {
                ...
                const fixes = Array.isArray(parsed.fixes)
                    ? parsed.fixes.filter((item) => typeof item === 'string' && !!item.trim())
                    : undefined;
                insight = { rootCause, nextPrompt, fixes, raw: content };
                if (normalizedAdaptation)
                    this.applyAdaptations(normalizedAdaptation, 'grok-insight');
                if (normalizedActions.length)
                    await this.applySyncActions(normalizedActions, 'grok-insight');
            }
    ...
    setInsight(correlationId, insight) {
        if (this.grokInsights.has(correlationId))
            this.grokInsights.delete(correlationId);
